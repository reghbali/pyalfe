...
class DefaultALFEDataDir(PipelineDataDir):
    ...
    def get_output_image(
        ...
        >>> data_dir = DefaultALFEDataDir(output_dir='/mri-data/processed', input_dir='/mri-data/input')
        ...
        >>> data_dir.get_output_image('34521', 'T1Post', resampling_target='T1Post', resampling_origin='template')
        '/mri-data/processed/34521/T1Post/34521_template_to_T1Post.nii.gz'

        >>> data_dir.get_output_image('34521', 'T2', image_type='abnormal_seg', sub_dir_name='abnormalmap')
    ...
    def get_input_image(self, accession, modality, extension='.nii.gz'):
        ...
        """
        return os.path.join(
            self('input', accession, modality), f'{modality}.nii.gz'
        )
    ...
    def get_quantification_file(
        ...
        >>> from pyalfe.data_structure import DefaultALFEDataDir
        >>> data_dir = DefaultALFEDataDir(output_dir='/mri-data/processed', input_dir='/mri-data/input')
        >>> data_dir.get_quantification_file('34521', 'T1Post', quantification_file_type='IndividualLesionMeasures')
...
class BIDSDataDir(PipelineDataDir):
    ...
    def __init__(self, output_dir, input_dir):
        ...
        with open(Path(output_dir).expanduser() / 'dataset_description.json',
                  'w') as file:
            json.dump(
                {'Name': 'ALFE Output',
                 'BIDSVersion': '1.0.2',
    ...
    def get_input_image(self, accession, modality, extension='.nii.gz'):
        ...
        >>> data_dir = BIDSDataDir(output_dir='/mri-data/processed', input_dir='/mri-data/input')
        >>> data_dir.get_input_image('34521', 'T1')
    ...
    def get_output_image(
        ...
        ret = self.output_layout.build_path(entities, pattern,
                                            validate=False)
