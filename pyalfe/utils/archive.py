...
def copy_matching_paths(src_pattern, dst, root_dir=''):
    if type(src_pattern) is str:
        ...
        raise ValueError(f'src_pattern should be either str or an Iterable.')
...
def extract_file_from_archive(archive_path, dst, relative_file_path):
    ...
    if archive_type == 'dmg':
        ...
        extract_file_from_tar(
            archive_path, extract_path, dst, relative_file_path)
    else:
        raise ValueError(f'cannot extract from {archive_path}'
...
def extract_file_from_dmg(dmg_path, mount_path, dst, relative_file_path):
    cdr_path = os.path.join(
        '/tmp', os.path.basename(dmg_path).split('.')[0] + '.cdr')

    subprocess.run(
        ['hdiutil', 'convert',
         dmg_path, '-format', 'UDTO', '-o', cdr_path])
