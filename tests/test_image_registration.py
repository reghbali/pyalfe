...
from pyalfe.image_registration import ImageRegistration, GreedyRegistration, \
    AntsRegistration
from tests.utils import create_nifti, get_nifti_data


def get_image_registration_test(image_registration: ImageRegistration) -> Type[TestCase]:
    class TestImageRegistration(TestCase):
        ...
        def test_register_rigid(self):
            ...
            image_registration.register_rigid(
                fixed_path, moving_path, transform)

            output = self.get_image_path('output.nii.gz')
            image_registration.reslice(
                fixed_path, moving_path, output, transform)

            np.testing.assert_array_less(
                np.linalg.norm(get_nifti_data(output) - fixed_data) /
                np.linalg.norm(fixed_data), 0.15)
            ...
            image_registration.register_rigid(
                fixed_path, moving2_path, transform2)

            output2 = self.get_image_path('output2.nii.gz')
            image_registration.reslice(fixed_path, moving2_path, output2,
                                       transform2)

            np.testing.assert_array_less(
                np.linalg.norm(get_nifti_data(output2) - fixed_data) /
                np.linalg.norm(get_nifti_data(output2) - moving2_data), 0.25)
        ...
        def test_register_affine(self):
            ...
            output = self.get_image_path('output.nii.gz')
            image_registration.reslice(fixed_path, moving_path, output,
                                       transform)
            ...
            np.testing.assert_array_less(
                np.linalg.norm(get_nifti_data(output2) - fixed_data) /
                np.linalg.norm(get_nifti_data(output2) - moving2_data), 0.30)

            moving3_path = self.get_image_path('moving3.nii.gz')
            moving3_data = scipy.ndimage.zoom(
                fixed_data, 1.2)
            ...
            image_registration.reslice(fixed_path, moving3_path, output3,
                                       transform3)
