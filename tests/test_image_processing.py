...
from pyalfe.image_processing import ImageProcessor, Convert3DProcessor, \
    NilearnProcessor
...
def get_image_processor_test(image_processor: ImageProcessor) -> Type[TestCase]:
    ...
    class TestImageProcessor(TestCase):
        ...
        def test_holefill(self):
            ...
            np.testing.assert_array_equal(
                get_nifti_data(output), np.ones([3, 3, 3]))
        ...
        def test_reslice_to_ref(self):
            ...
            image_processor.reslice_to_ref(
                fixed_path, moving_path, output)
            output_nifti = get_nifti(output)
            np.testing.assert_array_equal(
                output_nifti.get_fdata().shape,
                fixed_data.shape)
            np.testing.assert_allclose(
                output_nifti.affine, affine
            )
        ...
        def test_resample_new_dim(self):
            ...
            image_processor.resample_new_dim(
                input_image_path, output, 100, 100, 200)
            np.testing.assert_array_equal(
                get_nifti_data(output).shape, (16, 16, 32))

            output2 = self.get_image_path('output2.nii.gz')
            image_processor.resample_new_dim(
                input_image_path, output2, 100, 50, 100)
            np.testing.assert_array_equal(
                get_nifti_data(output2).shape, (16, 8, 16))

            output3 = self.get_image_path('output3.nii.gz')
            image_processor.resample_new_dim(
                input_image_path, output3, 400, 100, 100)
            np.testing.assert_array_equal(
                get_nifti_data(output3).shape, (64, 16, 16))

            output4 = self.get_image_path('output4.nii.gz')
            image_processor.resample_new_dim(
                input_image_path, output4, 32, 23, 22, percent=False)
            np.testing.assert_array_equal(
                get_nifti_data(output4).shape, (32, 23, 22))
        ...
        def test_get_dims(self):
            ...
            np.testing.assert_array_equal(
                image_processor.get_dims(input_image_path),
                (10, 20, 29))
        ...
        def test_trim_largest_comp(self):
            ...
            output2 = self.get_image_path('output2.nii.gz')
            image_processor.trim_largest_comp(
                input_image_path, output2, (0, 0, 1))
            np.testing.assert_array_equal(
            ...
            output3 = self.get_image_path('output3.nii.gz')
            image_processor.trim_largest_comp(
                input_image_path, output3, (0, 0, 2))
            np.testing.assert_array_equal(
            ...
            output4 = self.get_image_path('output4.nii.gz')
            image_processor.trim_largest_comp(
                input_image_path, output4, (0, 0, 3))
            np.testing.assert_array_equal(
        ...
        def test_set_subtract(self):
            ...
            image_processor.set_subtract(
                minuend_path, subtrahend_path, output)
            np.testing.assert_array_equal(
